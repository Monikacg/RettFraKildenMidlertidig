...
/*
This module is an submodule of admin. It is used mainly to calculate if we should stop when we arrive at a floor,
or where we should go next.

Three functions are used directly by admin:
- CalculateNextOrder
- GetNewDirection
- ShouldStop
*/
...
func amIClosestToOrder(orders [][]int, properties []int, aliveLifts []int, ID int) (int, bool) {
	...
	for floor := 0; floor < N_FLOORS; floor++ {
		...
		if orders[BUTTON_CALL_UP][floor] == 0 {
			if !(orders[BUTTON_CALL_DOWN][floor] > 0) { // In case someone else has button down assigned. To avoid sending a lift to the same floor as a different lift.
	...
	// then that lift should take this order, and whoever called this function should not go to that floor.
	for _, lift := range aliveLifts {
		_, destinationExists := checkForValidDestination(orders, lift)
		if !destinationExists {
