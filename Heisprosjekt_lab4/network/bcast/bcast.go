...
func Transmitter(port int, id string, chans ...interface{}) {
	...
	for {
		...
		conn.WriteTo([]byte(id+":"+typeNames[chosen]+string(buf)), addr)
	}
}

// Matches type-tagged JSON received on `port` to element types of `chans`, then
// sends the decoded value on the corresponding channel
func Receiver(port int, id string, recvFromSelf bool, chans ...interface{}) {
	checkArgs(chans...)
	...
	for {
		...
		m := strings.Index(string(buf[0:]), ":")
		remoteID := string(buf[0:m])
		if recvFromSelf || remoteID != id {
			buf2 := buf[m+2:n] //dobbeltsjekke 2
			for _, ch := range chans {
				T := reflect.TypeOf(ch).Elem()
				typeName := T.String()
				if strings.HasPrefix(string(buf2)+"{", typeName) {

					v := reflect.New(T)
					json.Unmarshal(buf2[len(typeName):], v.Interface())
