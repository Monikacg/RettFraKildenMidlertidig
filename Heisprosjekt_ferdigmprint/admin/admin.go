			...
			if ownID != backupMsg.SenderID {
				if !backupsAreIdentical(backupMsg, lastBackUpRecevied[backupMsg.SenderID]) {
					...
					case "I was isolated":
						...
						// Will take the highest valued inner order it can find (assigned order > existing order > no order)
					...
					case "I was part of a group":
						...
						fmt.Println("Adm: Orders before backupcommands: ", orders)
						// Will take the highest valued inner order it can find (assigned order > existing order > no order), and copy the outer orders.
