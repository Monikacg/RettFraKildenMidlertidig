...
func checksIncomingMessages(IDInput int, sendAliveLiftListToCheckIncomingMessagesChan <-chan Lifts, messagesToSendChan <-chan Message, messageReceiveChan <-chan Broadcast,
	...
	lastSeqsStart := make([]int, MAX_N_LIFTS) // TO REMOVE PROBLEM WHEN ONE LIFT GETS BACK ONLINE.
		...
		case recvMsg := <-messageReceiveChan:
			...
			if recvMsg.ThisIsAnAck {
				if recvMsg.Message.ID == ownID {
					// Someone (has to be someone else) sending an ack on a message we sent out.
					for i, ack := range msgAcks {
				...
				if recvMsg.Message.ID != ownID {
					...
					fmt.Println("NW: Got message from someone else. Sender ID::SequenceStart::SequenceNumber::seqs::lastSeqs at beginning", recvMsg.Message.ID, recvMsg.SequenceStart, recvMsg.SequenceNumber, seqs, lastSeqsStart)
					...
					fmt.Println("NW: After check: seqs::lastSeqs", seqs, lastSeqsStart)
